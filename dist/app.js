...
import { requireRole, createRequireAuth } from "./middleware/auth.js";
import { validateCommonProfile, validateInfluencerProfile } from "./validation.js";
const onboardingRoles = ["manager", "user", "influencer"];
export function createApp(database, verifyToken) {
    const app = express();
    ...
    app.get("/api/me", requireAuth, (req, res) => res.status(200).json({ status: "ok", user: req.user }));
    ...
    app.put("/api/onboarding/me/profile", requireAuth, requireRole(...onboardingRoles), async (req, res) => {
        ...
        const result = await database.query(`INSERT INTO user_profiles (user_id, first_name, last_name, phone, timezone)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (user_id) DO UPDATE SET first_name = EXCLUDED.first_name,
         last_name = EXCLUDED.last_name, phone = EXCLUDED.phone, timezone = EXCLUDED.timezone,
         completed_at = NOW(), updated_at = NOW()
       RETURNING first_name, last_name, phone, timezone, completed_at`, [req.user?.id, first_name.trim(), last_name.trim(), phone, timezone]);
        res.status(200).json({ status: "ok", profile: result.rows[0] });
    });
    app.put("/api/onboarding/me/influencer", requireAuth, requireRole("influencer"), async (req, res) => {
        ...
        const result = await database.query(`INSERT INTO influencer_profiles
       ...
       RETURNING *`, [req.user?.id, values.display_name.trim(), values.bio ?? null, values.niche.trim(),
    ...
    app.get("/api/influencers", requireAuth, async (_req, res) => {
        const result = await database.query(`SELECT id, display_name, bio, niche, instagram_url, tiktok_url, youtube_url,
    ...
    app.patch("/api/admin/users/:id/role", requireAuth, requireRole("admin"), async (req, res) => {
        ...
        const validRoles = ["admin", "manager", "user", "influencer"];
        if (!validRoles.includes(requestedRole)) {
            res.status(400).json({ status: "error", message: "Invalid role. Role must be admin, manager, user, or influencer" });
            return;
        }
        if (String(req.user?.id) === req.params.id) {
            res.status(400).json({ status: "error", message: "Users cannot change their own role" });
            return;
        }
