    ...
    const database = {
        async query(text, values) {
            ...
            if (text.includes("FROM user_profiles"))
                return { rows: [] };
            if (text.includes("FROM influencer_profiles"))
                return { rows: [] };
            if (text.includes("INSERT INTO user_profiles"))
                return { rows: [{ first_name: "Ada", last_name: "Lovelace", completed_at: "2026-01-01T00:00:00Z" }] };
            if (text.includes("INSERT INTO influencer_profiles"))
                return { rows: [{ display_name: "Creator", niche: "Tech", completed_at: "2026-01-01T00:00:00Z" }] };
            return { rows: [] };
        },
    };
    const verifyToken = (async (token) => ({ uid: token }));
...
test("allows all authenticated roles to list influencers", async () => {
    for (const role of ["admin", "manager", "user", "influencer"]) {
        const { app } = createTestApp();
